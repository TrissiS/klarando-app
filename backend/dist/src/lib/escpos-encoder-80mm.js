...
exports.encodeReceiptToEscPosBytes = encodeReceiptToEscPosBytes;
const ESC = 0x1b;
const GS = 0x1d;
const CODEPAGE_TO_ESC_T = {
    CP437: 0,
    CP850: 2,
    CP1252: 16,
    CP858: 19,
...
function fontByte(scale) {
    return scale === 'S' ? 1 : 0;
}
function sizeByte(scale) {
    if (scale === 'L') {
        return 0x11;
    }
    if (scale === 'XL') {
...
function pushArray(target, values) {
    target.push(...values);
}
...
function encodeLine(target, text) {
    pushArray(target, encodeTextBytes(text));
    target.push(0x0a);
}
function encodeQr(target, value, size) {
...
function encodeCommand(target, command) {
...
function encodeReceiptToEscPosBytes(renderResult) {
    const bytes = [];
    // Initialize printer.
