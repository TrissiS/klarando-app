...
function normalizeStringListInput(input) {
    if (input === undefined) {
        return undefined;
    }
    if (input === null) {
    ...
    const rawItems = Array.isArray(input) ? input : input.split(/[\n,]+/g);
    const values = Array.from(new Set(rawItems
        .map((entry) => entry.trim())
        .filter((entry) => entry.length > 0)));
    return values;
}
...
function isActionCurrentlyActiveForFeed(action, now = new Date()) {
    if (!action.isActive) {
        return false;
    }
...
function collectSizeOptions(product) {
    ...
    return product.modifiers
        ...
        .map((modifier) => {
        const parsed = (0, product_modifiers_1.decodeStoredProductModifierName)(modifier.name);
...
function sortScreenProducts(left, right, mode = 'CUSTOM') {
...
router.put('/config', (0, auth_1.requirePermission)(client_1.PermissionKey.SETTINGS_WRITE), async (req, res) => {
    try {
        const { tenantId, title, subtitle, fontFamily, defaultOrientation, defaultResolutionWidth, defaultResolutionHeight, productFontSize, ingredientFontSize, categoryFontSize, priceFontSize, backgroundMode, backgroundValue, backgroundMediaUrl, overlayAnimation, cardStyle, cardPadding, cardBackgroundOpacity, cardBorderOpacity, productSortMode, defaultColumnCount, showCategoryHeaders, showCategoryOnCard, categoryUppercase, accentColor, textColor, productNameColor, ingredientTextColor, categoryTextColor, priceTextColor, showAllergens, allergenLegendEnabled, allergenLegendPosition, showPrices, pricePinnedLeft, priceDisplayMode, showUnavailable, showProductNumber, logoUrl, logoSize, offerWindowEnabled, offerWindowTitle, offerWindowText, offerWindowPosition, offerWindowSize, offerWindowWidthPx, offerWindowHeightPx, offerWindowXPercent, offerWindowYPercent, offerWindowReserveSpace, offerWindowBackgroundColor, offerWindowTextColor, offerWindowOpacity, offerMediaUrls, offerMediaRotateSec, tickerEnabled, tickerPosition, tickerShowClock, tickerDataSource, tickerCustomText, tickerApiUrl, tickerRefreshSec, tickerClockFontSize, tickerFontSize, tickerInsetPx, tickerOffsetPx, tickerBackgroundColor, tickerTextColor, } = req.body;
        if (!tenantId) {
            return res.status(400).json({ error: 'tenantId fehlt' });
        }
        ...
        if (defaultResolutionWidth !== undefined &&
        ...
        if (defaultResolutionHeight !== undefined &&
        ...
        if (offerWindowXPercent !== undefined &&
        ...
        if (offerWindowYPercent !== undefined &&
        ...
        if (ingredientTextColor !== undefined &&
        ...
        if (categoryTextColor !== undefined &&
        ...
        if (offerWindowBackgroundColor !== undefined &&
        ...
        if (offerWindowTextColor !== undefined &&
        ...
        if (tickerClockFontSize !== undefined &&
        ...
        if (tickerBackgroundColor !== undefined &&
        ...
        const normalizedTickerApiUrlForValidation = tickerApiUrl === undefined ? undefined : normalizeText(tickerApiUrl);
        ...
        const parsedProductFontSize = productFontSize === undefined ? undefined : clampInt(Number(productFontSize), 16, 96);
        const parsedIngredientFontSize = ingredientFontSize === undefined ? undefined : clampInt(Number(ingredientFontSize), 10, 48);
        const parsedCategoryFontSize = categoryFontSize === undefined ? undefined : clampInt(Number(categoryFontSize), 10, 48);
        const parsedPriceFontSize = priceFontSize === undefined ? undefined : clampInt(Number(priceFontSize), 14, 72);
        const parsedCardPadding = cardPadding === undefined ? undefined : clampInt(Number(cardPadding), 8, 48);
        const parsedCardBackgroundOpacity = cardBackgroundOpacity === undefined
            ? undefined
            : clampInt(Number(cardBackgroundOpacity), 0, 100);
        const parsedCardBorderOpacity = cardBorderOpacity === undefined ? undefined : clampInt(Number(cardBorderOpacity), 0, 100);
        const parsedDefaultColumnCount = defaultColumnCount === undefined ? undefined : clampInt(Number(defaultColumnCount), 1, 6);
        const parsedLogoSize = logoSize === undefined ? undefined : clampInt(Number(logoSize), 32, 220);
        const parsedOfferMediaRotateSec = offerMediaRotateSec === undefined ? undefined : clampInt(Number(offerMediaRotateSec), 3, 120);
        const parsedOfferWindowWidthPx = offerWindowWidthPx === undefined ? undefined : clampInt(Number(offerWindowWidthPx), 180, 1400);
        const parsedOfferWindowHeightPx = offerWindowHeightPx === undefined
            ? undefined
            : clampInt(Number(offerWindowHeightPx), 120, 1200);
        const parsedOfferWindowOpacity = offerWindowOpacity === undefined
            ? undefined
            : clampInt(Number(offerWindowOpacity), 0, 100);
        const parsedDefaultResolutionWidth = defaultResolutionWidth === undefined
            ? undefined
            : clampInt(Number(defaultResolutionWidth), 320, 10000);
        const parsedDefaultResolutionHeight = defaultResolutionHeight === undefined
            ? undefined
            : clampInt(Number(defaultResolutionHeight), 240, 10000);
        const parsedOfferWindowXPercent = offerWindowXPercent === undefined
            ? undefined
            : clampInt(Number(offerWindowXPercent), 0, 100);
        const parsedOfferWindowYPercent = offerWindowYPercent === undefined
            ? undefined
            : clampInt(Number(offerWindowYPercent), 0, 100);
        const parsedTickerRefreshSec = tickerRefreshSec === undefined ? undefined : clampInt(Number(tickerRefreshSec), 10, 3600);
        const parsedTickerClockFontSize = tickerClockFontSize === undefined ? undefined : clampInt(Number(tickerClockFontSize), 10, 48);
        const parsedTickerFontSize = tickerFontSize === undefined ? undefined : clampInt(Number(tickerFontSize), 10, 42);
        const parsedTickerInsetPx = tickerInsetPx === undefined ? undefined : clampInt(Number(tickerInsetPx), 0, 420);
        const parsedTickerOffsetPx = tickerOffsetPx === undefined ? undefined : clampInt(Number(tickerOffsetPx), 0, 260);
        const normalizedOfferMediaUrls = offerMediaUrls === undefined ? undefined : normalizeStringListInput(offerMediaUrls);
        const normalizedFontFamily = fontFamily === undefined ? undefined : normalizeText(fontFamily) ?? 'Poppins, sans-serif';
        const normalizedTickerCustomText = tickerCustomText === undefined ? undefined : normalizeText(tickerCustomText);
        const normalizedTickerApiUrl = tickerApiUrl === undefined ? undefined : normalizeText(tickerApiUrl);
        const config = await prisma_1.prisma.screenSetting.upsert({
            where: { tenantId },
            update: {
                ...
                productNameColor: productNameColor === undefined
                    ? undefined
                    : normalizeHexColor(productNameColor) ?? '#ffffff',
                ingredientTextColor: ingredientTextColor === undefined
                    ? undefined
                    : normalizeHexColor(ingredientTextColor) ?? '#e2e8f0',
                categoryTextColor: categoryTextColor === undefined
                    ? undefined
                    : normalizeHexColor(categoryTextColor) ?? '#cbd5e1',
                priceTextColor: priceTextColor === undefined
                    ? undefined
                ...
                offerWindowReserveSpace,
                offerWindowBackgroundColor: offerWindowBackgroundColor === undefined
                    ? undefined
                    : normalizeHexColor(offerWindowBackgroundColor) ?? '#f97316',
                offerWindowTextColor: offerWindowTextColor === undefined
                    ? undefined
                    : normalizeHexColor(offerWindowTextColor) ?? '#ffffff',
                offerWindowOpacity: parsedOfferWindowOpacity,
                offerMediaUrls: normalizedOfferMediaUrls === undefined ? undefined : normalizedOfferMediaUrls ?? [],
                ...
                tickerBackgroundColor: tickerBackgroundColor === undefined
                    ? undefined
                    : normalizeHexColor(tickerBackgroundColor) ?? '#111827',
                tickerTextColor: tickerTextColor === undefined ? undefined : normalizeHexColor(tickerTextColor) ?? '#ffffff',
            },
            create: {
                tenantId,
                ...
                defaultOrientation: defaultOrientation && ORIENTATIONS.has(defaultOrientation)
                    ? defaultOrientation
                    : 'LANDSCAPE',
                ...
                productSortMode: productSortMode && PRODUCT_SORT_MODES.has(productSortMode)
                    ? productSortMode
                    : 'CUSTOM',
                ...
                priceDisplayMode: priceDisplayMode && PRICE_DISPLAY_MODES.has(priceDisplayMode)
                    ? priceDisplayMode
                    : 'EUR_SUFFIX',
                ...
                offerWindowPosition: offerWindowPosition && OFFER_WINDOW_POSITIONS.has(offerWindowPosition)
                    ? offerWindowPosition
                    : 'RIGHT',
                offerWindowSize: offerWindowSize && OFFER_WINDOW_SIZES.has(offerWindowSize) ? offerWindowSize : 'M',
                ...
                tickerPosition: tickerPosition && TICKER_POSITIONS.has(tickerPosition) ? tickerPosition : 'BOTTOM',
                tickerShowClock: tickerShowClock ?? true,
                tickerDataSource: tickerDataSource && TICKER_DATA_SOURCES.has(tickerDataSource)
                    ? tickerDataSource
                    : 'CUSTOM',
...
router.post('/devices', (0, auth_1.requirePermission)(client_1.PermissionKey.SETTINGS_WRITE), async (req, res) => {
    try {
        const { tenantId, name, deviceCode, location, orientation, resolutionWidth, resolutionHeight, refreshIntervalSec, isActive, showPricesOverride, showAllergensOverride, showUnavailableOverride, showProductNumberOverride, offerWindowEnabledOverride, offerWindowPositionOverride, offerWindowSizeOverride, offerMediaUrlsOverride, offerMediaRotateSecOverride, cardStyleOverride, columnCountOverride, selectedCategoryIds, selectedProductIds, backgroundMediaUrlOverride, accentColorOverride, textColorOverride, notes, } = req.body;
        if (!tenantId || !normalizeText(name)) {
            return res.status(400).json({ error: 'tenantId und name sind erforderlich' });
        }
        const globalDefaults = await prisma_1.prisma.screenSetting.upsert({
            where: { tenantId },
            update: {},
            create: { tenantId },
        });
        ...
        const width = resolutionWidth === undefined
            ? Number(globalDefaults.defaultResolutionWidth || 1920)
            : Number(resolutionWidth);
        const height = resolutionHeight === undefined
        ...
        if (offerWindowPositionOverride !== undefined &&
        ...
        if (offerWindowSizeOverride !== undefined &&
        ...
        if (cardStyleOverride !== undefined &&
        ...
        const normalizedCategoryIds = selectedCategoryIds === undefined ? undefined : normalizeIdList(selectedCategoryIds);
        const normalizedProductIds = selectedProductIds === undefined ? undefined : normalizeIdList(selectedProductIds);
        const normalizedOfferMediaUrlsOverride = offerMediaUrlsOverride === undefined
            ? undefined
        ...
        const parsedColumnCountOverride = columnCountOverride === undefined
            ? undefined
            : columnCountOverride === null
                ? null
                : clampInt(Number(columnCountOverride), 1, 6);
        const parsedOfferMediaRotateSecOverride = offerMediaRotateSecOverride === undefined
            ? undefined
...
router.patch('/devices/:id', (0, auth_1.requirePermission)(client_1.PermissionKey.SETTINGS_WRITE), async (req, res) => {
    try {
        ...
        const { tenantId, name, location, orientation, resolutionWidth, resolutionHeight, refreshIntervalSec, isActive, showPricesOverride, showAllergensOverride, showUnavailableOverride, showProductNumberOverride, offerWindowEnabledOverride, offerWindowPositionOverride, offerWindowSizeOverride, offerMediaUrlsOverride, offerMediaRotateSecOverride, cardStyleOverride, columnCountOverride, selectedCategoryIds, selectedProductIds, backgroundMediaUrlOverride, accentColorOverride, textColorOverride, notes, } = req.body;
        if (!id || !tenantId) {
        ...
        const updated = await prisma_1.prisma.screenDevice.update({
            ...
            data: {
                ...
                showProductNumberOverride: showProductNumberOverride === undefined ? undefined : showProductNumberOverride,
                offerWindowEnabledOverride: offerWindowEnabledOverride === undefined
                    ? undefined
                    : offerWindowEnabledOverride,
                offerWindowPositionOverride: offerWindowPositionOverride === undefined ? undefined : offerWindowPositionOverride,
                offerWindowSizeOverride: offerWindowSizeOverride === undefined ? undefined : offerWindowSizeOverride,
                offerMediaUrlsOverride: offerMediaUrlsOverride === undefined
                    ? undefined
                    : normalizedOfferMediaUrlsOverride ?? [],
                offerMediaRotateSecOverride: parsedOfferMediaRotateSecOverride,
                cardStyleOverride: cardStyleOverride === undefined ? undefined : cardStyleOverride,
                ...
                selectedProductIds: selectedProductIds === undefined
                    ? undefined
...
router.get('/public/devices/:deviceCode/feed', async (req, res) => {
    try {
        ...
        const loadActiveActions = () => prisma_1.prisma.action.findMany({
            where: {
                tenantId: device.tenantId,
                isActive: true,
            },
        ...
        const activeActions = actionsRaw
            ...
            const priceLabel = action.hidePriceOnScreen
                ? null
                : action.valueType === client_1.ActionValueType.FIXED_PRICE
        ...
        const effectiveConfig = {
            ...
            showProductNumber: device.showProductNumberOverride === null
            ...
            offerWindowPosition: device.offerWindowPositionOverride === null
                ? baseConfig.offerWindowPosition
                : device.offerWindowPositionOverride,
            offerWindowSize: device.offerWindowSizeOverride === null
                ? baseConfig.offerWindowSize
                : device.offerWindowSizeOverride,
            offerMediaUrls: device.offerMediaUrlsOverride === null ? baseOfferMediaUrls : deviceOfferMediaUrls,
            offerMediaRotateSec: device.offerMediaRotateSecOverride === null
                ? baseConfig.offerMediaRotateSec
                : device.offerMediaRotateSecOverride,
            cardStyle: device.cardStyleOverride === null ? baseConfig.cardStyle : device.cardStyleOverride,
            defaultColumnCount: device.columnCountOverride === null
        ...
        const products = productsRaw
            ...
            .sort((left, right) => sortScreenProducts(left, right, effectiveConfig.productSortMode));
        const announcementNow = new Date();
        const pickupAnnouncementsRaw = await prisma_1.prisma.order.findMany({
            where: {
                tenantId: device.tenantId,
                status: 'done',
                pickupNumber: {
