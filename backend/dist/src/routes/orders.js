...
const MAX_COMPLAINT_IMAGE_PAYLOAD = 2500000;
const MAX_SIGNATURE_IMAGE_PAYLOAD = 3000000;
const DRIVER_DEVICE_ONLINE_HEARTBEAT_MS = 90000;
...
function readAppBearerToken(authorizationHeader) {
...
function isOrderAssignedToDriverActor(order, actor) {
...
function parseDriverDevicePairingToken(value) {
    if (typeof value !== 'string') {
        return null;
    }
...
function parseDriverPairPayload(value) {
...
async function resolveDriverActor(req) {
    ...
    if (authUser && authUser.role === client_1.UserRole.DRIVER && authUser.tenantId) {
        return {
            actorType: 'USER',
    ...
    const tokenPayload = (0, driver_device_token_1.verifyDriverDeviceToken)(bearerToken);
    if (!tokenPayload || tokenPayload.kind !== 'SESSION') {
        return null;
    }
    const session = await (0, driver_device_sessions_1.resolveDriverDeviceSession)(tokenPayload.sid, tokenPayload.tenantId);
    ...
    if (tokenPayload.driverUserId) {
        ...
        if (!driver ||
            !driver.isActive ||
            driver.role !== client_1.UserRole.DRIVER ||
        ...
        return {
            ...
            driverName: normalizeText(tokenPayload.driverName) ??
    ...
    const resolvedDriverName = normalizeText(tokenPayload.driverName) ??
        normalizeText(session.driverName) ??
...
async function resolveAppAccountFromAuthorizationHeader(authorizationHeader) {
...
function parseRatingComment(value) {
    if (typeof value !== 'string') {
        return null;
    }
    const trimmed = value.trim();
...
function parseCommentFromAuditMetadata(metadata) {
    if (!metadata || typeof metadata !== 'object') {
        return null;
    }
...
function parseTenantIdQuery(value) {
    if (typeof value !== 'string') {
        return null;
    }
    const trimmed = value.trim();
...
function sanitizeImagePayload(input) {
...
function parseSignaturePayloadFromAuditMetadata(metadata) {
    if (!metadata || typeof metadata !== 'object' || Array.isArray(metadata)) {
        return null;
    }
    const source = metadata;
...
async function loadOrderIssueStateByOrderId(orderIds) {
    const normalizedIds = Array.from(new Set(orderIds.filter(Boolean)));
...
async function loadOrderForCreateResponse(orderId) {
...
function normalizeStockMatchingKey(value) {
    const normalized = normalizeText(value);
...
function shouldDeductInventoryForStatus(status) {
    return (status === 'open' ||
        status === 'preparing' ||
...
async function applyInventoryConsumptionForOrder(tx, orderId) {
        ...
        if (beverageProducts.length > 0) {
            ...
            if (eanCandidates.length > 0 || nameCandidates.length > 0) {
                ...
                for (const product of beverageProducts) {
                    ...
                    const matchedIngredient = (eanKey ? ingredientByEan.get(eanKey) : undefined) ??
    ...
    for (const orderItem of order.items) {
        ...
        for (const line of recipe) {
            ...
            const consumptionFactorPercent = Math.max(0, Number(line.ingredient.consumptionFactorPercent ?? 0));
            const quantityForItem = convertedQuantity * orderItem.quantity * (1 + consumptionFactorPercent / 100);
...
async function resolveScopedTenantIdsForRatings(requestedTenantId, authUser) {
    if (!authUser) {
        return [];
    }
    if (authUser.role === client_1.UserRole.SUPERADMIN) {
    ...
    if (authUser.role === client_1.UserRole.CHAINADMIN) {
        if (!authUser.chainId) {
            return [];
        }
        const tenants = await prisma_1.prisma.tenant.findMany({
...
async function resolveScopedTenantIdsForOrderManagement(input) {
    ...
    if ((authUser.role === client_1.UserRole.ADMIN || authUser.role === client_1.UserRole.STAFF) && authUser.tenantId) {
        if (requestedTenantId && requestedTenantId !== authUser.tenantId) {
            return [];
        }
        return [authUser.tenantId];
...
function parseDashboardDays(rawValue) {
...
function resolveDashboardRange(days) {
    const to = new Date();
...
function resolveOrderRatingWindow(paidAt, now = new Date()) {
...
async function collectPendingRatingStatsLast72h(tenantIds) {
    ...
    const paidOrders = await prisma_1.prisma.order.findMany({
        where: {
            tenantId: {
                in: tenantIds,
            },
    ...
    const ratedRows = await prisma_1.prisma.auditLog.findMany({
        where: {
            module: 'order_rating',
    ...
    const ratedOrderIds = new Set(ratedRows.map((entry) => entry.targetId).filter((entry) => Boolean(entry)));
    let unratedTotal = 0;
...
router.get('/', async (req, res) => {
    try {
        ...
        const channel = normalizeText(req.query.channel)?.toUpperCase() ?? null;
        const appOnly = channel === 'APP_ONLY';
        const authUser = req.authUser;
        const appAccount = await resolveAppAccountFromAuthorizationHeader(req.header('authorization') || undefined);
        ...
        if (appAccount) {
            ...
            if (authUser.role !== client_1.UserRole.SUPERADMIN &&
                !authUser.permissions.has(client_1.PermissionKey.ORDERS_READ)) {
                return res.status(403).json({ error: `Keine Berechtigung: ${client_1.PermissionKey.ORDERS_READ}` });
            }
            if (tenantId) {
        ...
        const latestDriverLocations = await loadLatestDriverLocationByOrderId(orders.map((entry) => entry.id));
...
router.get('/management', (0, auth_1.requirePermission)(client_1.PermissionKey.ORDERS_READ), async (req, res) => {
    try {
        if (!req.authUser) {
            return res.status(401).json({ error: 'Nicht eingeloggt' });
        }
        ...
        const source = normalizeText(req.query.source)?.toUpperCase() ?? 'ALL';
...
router.get('/dashboard/admin', (0, auth_1.requirePermission)(client_1.PermissionKey.ORDERS_READ), async (req, res) => {
    try {
        ...
        const productStatsMap = new Map(products.map((product) => [
            product.id,
            {
...
router.get('/dashboard/superadmin', (0, auth_1.requirePermission)(client_1.PermissionKey.ORDERS_READ), async (req, res) => {
    try {
        if (!req.authUser || req.authUser.role !== client_1.UserRole.SUPERADMIN) {
...
router.get('/ratings/admin', (0, auth_1.requirePermission)(client_1.PermissionKey.ORDERS_READ), async (req, res) => {
    try {
        const requestedTenantId = parseTenantIdQuery(req.query.tenantId);
        const periodDays = parseDashboardDays(req.query.days);
        ...
        const ratings = await prisma_1.prisma.auditLog.findMany({
            where: {
                module: 'order_rating',
                action: 'order_rating_submitted',
                tenantId: {
...
router.get('/ratings/superadmin', (0, auth_1.requirePermission)(client_1.PermissionKey.ORDERS_READ), async (req, res) => {
    try {
        if (!req.authUser || req.authUser.role !== client_1.UserRole.SUPERADMIN) {
            return res.status(403).json({ error: 'Nur SUPERADMIN erlaubt' });
        }
        ...
        const latest = ratings
            ...
            .filter((entry) => entry !== null)
            .slice(0, 150);
        const tenantsForPending = await prisma_1.prisma.tenant.findMany({
            select: {
                id: true,
            },
        });
        const pendingRatings72h = await collectPendingRatingStatsLast72h(tenantsForPending.map((entry) => entry.id));
        return res.json({
            periodDays,
            from: from.toISOString(),
            totalRatings,
...
router.post('/', rate_limit_1.rateLimitPublicOrderCreate, async (req, res) => {
    let idempotencyReservation = null;
    try {
        const { tenantId, terminalId, items, sourceChannel, paymentMethod, markPaid, forwardToKitchen, serviceType, customerName, customerPhone, customerAddress, customerZipCode, customerCity, idempotencyKey, } = req.body;
        ...
        const normalizedSourceChannel = (normalizeText(sourceChannel)?.toUpperCase() ??
        ...
        const resolvedServiceType = normalizedServiceTypeRaw === 'DELIVERY' || normalizedServiceTypeRaw === 'PICKUP'
        ...
        if (!isAppOrderChannel) {
            ...
            if (actor.role !== client_1.UserRole.SUPERADMIN &&
                !actor.permissions.has(client_1.PermissionKey.ORDERS_WRITE)) {
                return res.status(403).json({ error: `Keine Berechtigung: ${client_1.PermissionKey.ORDERS_WRITE}` });
            }
        }
        const resolvedTenantId = isAppOrderChannel
            ? tenantId
            : (await (0, tenant_scope_1.resolveTenantScope)(req, tenantId)).tenantId;
        ...
        if (normalizedTerminalId) {
            terminal = await prisma_1.prisma.orderTerminal.findUnique({
        ...
        const invalidQuantityItem = items.find((item) => !Number.isInteger(item.quantity) || item.quantity <= 0 || item.quantity > 200);
        ...
        const offeredPrices = await (0, action_pricing_1.resolveProductOffers)(resolvedTenantId, products.map((entry) => ({
            id: entry.id,
        ...
        const requestedModifierIds = Array.from(new Set(items.flatMap((item) => Array.isArray(item.modifierIds)
            ? item.modifierIds
                .map((entry) => normalizeText(entry))
                .filter((entry) => Boolean(entry))
            : [])));
        const modifiers = requestedModifierIds.length > 0
        ...
        const orderItemsData = items.map((item) => {
            ...
            const requestedIdsForItem = Array.isArray(item.modifierIds)
                ? Array.from(new Set(item.modifierIds
                    .map((entry) => normalizeText(entry))
                    .filter((entry) => Boolean(entry))))
                : [];
            ...
            const unitBasePrice = offeredPrices.get(product.id)?.effectivePrice ?? Number(product.price);
            const unitModifierDelta = selectedModifiers.reduce((sum, entry) => sum + Number(entry.priceDelta), 0);
            ...
            return {
                ...
                modifierSnapshot: selectedModifiers.length > 0
                    ? selectedModifiers.map((entry) => ({
                        id: entry.id,
                        name: (0, product_modifiers_1.decodeStoredProductModifierName)(entry.name).displayName,
        ...
        const shouldMarkPaid = normalizedPaymentMethod === 'PAYPAL' || normalizedPaymentMethod === 'KLARNA'
            ? true
            : Boolean(markPaid);
        ...
        if (isAppOrderChannel) {
            ...
            if (resolvedServiceType === 'DELIVERY') {
                ...
                if (minOrderAmount !== null && subtotal < minOrderAmount) {
                    return res.status(400).json({
                        error: `Mindestbestellwert nicht erreicht. Aktuell ${subtotal.toFixed(2)} EUR, erforderlich ${minOrderAmount.toFixed(2)} EUR.`,
                    });
                }
        ...
        const localPickupNumber = shouldAssignLocalPickupNumber
            ? await (0, pickup_number_1.generateNextPickupNumberForTenant)(resolvedTenantId)
            : null;
...
router.post('/:orderId/rating', async (req, res) => {
    try {
        ...
        const stars = parseRatingStars(req.body.stars);
        const comment = parseRatingComment(req.body.comment);
        const appAccount = await resolveAppAccountFromAuthorizationHeader(req.header('authorization') || undefined);
        if (!appAccount) {
        ...
        if (!ratingWindow.canRate) {
            if (ratingWindow.reason === 'cooldown') {
                return res.status(400).json({
                    error: `Bewertung ist 1 Stunde nach Zahlung moeglich (ab ${formatTimestamp(ratingWindow.opensAt)}).`,
                });
            }
            return res.status(400).json({
...
router.post('/:orderId/complaints/:complaintId/resolve', (0, auth_1.requirePermission)(client_1.PermissionKey.ORDERS_WRITE), async (req, res) => {
    try {
        if (!req.authUser) {
            return res.status(401).json({ error: 'Nicht eingeloggt' });
        }
        const orderId = normalizeText(Array.isArray(req.params.orderId) ? req.params.orderId[0] : req.params.orderId);
        const complaintId = normalizeText(Array.isArray(req.params.complaintId)
        ...
        const targetComplaint = issueState.complaints.find((entry) => entry.id === complaintId);
        if (!targetComplaint) {
...
router.post('/driver/device-login', rate_limit_1.rateLimitDriverDeviceLogin, async (req, res) => {
    try {
        ...
        const session = await (0, driver_device_sessions_1.resolveDriverDeviceSession)(pairingPayload.sid, pairingPayload.tenantId);
        if (!session || !session.isActive || session.revokedAt) {
        ...
        let boundDriverUserId = null;
        let boundDriverName = null;
        if (normalizedEmail || rawPassword.length > 0) {
            ...
            if (!driver ||
                !driver.isActive ||
                driver.role !== client_1.UserRole.DRIVER ||
                !(0, password_1.verifyPassword)(rawPassword, driver.passwordHash)) {
            ...
            if ((0, password_1.needsPasswordRehash)(driver.passwordHash)) {
                await prisma_1.prisma.user.update({
                    where: { id: driver.id },
                    data: {
                        passwordHash: (0, password_1.hashPassword)(rawPassword),
                    },
                });
            }
        ...
        const parsedSettings = (0, business_settings_1.parseSettings)(tenant.businessSettings, {
            name: tenant.name,
            email: tenant.email,
        });
        const accessHours = parseDriverAccessHours(payload.accessHours) ?? parsedSettings.driver.defaultAccessHours;
        const now = Date.now();
        ...
        const resolvedExpiryMs = Number.isFinite(maxSessionExpiryMs)
            ...
            : requestedExpiryMs;
        const sessionToken = (0, driver_device_token_1.signDriverDeviceToken)({
...
router.post('/driver/route-start', async (req, res) => {
    try {
        ...
        const now = new Date();
        const nextEstimatedMinutes = parsedEstimatedMinutes === null
...
router.get('/driver/assigned', async (req, res) => {
    try {
        ...
        const parsedTenantSettings = (0, business_settings_1.parseSettings)(tenant.businessSettings, {
            name: tenant.name,
            email: tenant.email,
        });
...
router.post('/driver/location', async (req, res) => {
    try {
        ...
        const { orderId, latitude, longitude, accuracyMeters, heading, speedKmh, } = req.body;
...
router.post('/:orderId/dispatch', (0, auth_1.requirePermission)(client_1.PermissionKey.ORDERS_WRITE), async (req, res) => {
    try {
        ...
        await (0, tenant_scope_1.resolveTenantScope)(req, currentOrder.tenantId);
        ...
        let resolvedDriverUser = null;
        let driverDeviceStatus = null;
        if (normalizedDriverUserId) {
            const foundDriver = await prisma_1.prisma.user.findUnique({
            ...
            if (!foundDriver || !foundDriver.isActive || foundDriver.role !== client_1.UserRole.DRIVER) {
            ...
            const driverSessions = await (0, driver_device_sessions_1.listDriverDeviceSessionsForTenant)(currentOrder.tenantId, {
                includeExpired: false,
            });
            const driverSession = driverSessions.find((entry) => entry.driverUserId === foundDriver.id && entry.isOnline) ??
        ...
        const nextEstimatedReadyAt = nextEstimatedMinutes && nextEstimatedMinutes > 0
        ...
        return res.json({
            ...updated,
            driverDeviceStatus,
            recoveryHint: driverDeviceStatus && !driverDeviceStatus.isOnline
...
router.patch('/:orderId/status', (0, auth_1.requirePermission)(client_1.PermissionKey.ORDERS_WRITE), async (req, res) => {
    try {
        const orderId = normalizeText(Array.isArray(req.params.orderId) ? req.params.orderId[0] : req.params.orderId);
