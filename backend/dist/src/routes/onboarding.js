...
function onboardingErrorResponse(res, status, error) {
...
async function nextUniqueChainCode(companyName) {
    const base = slugCode(companyName) || 'chain';
    let candidate = base;
    let attempt = 1;
    while (true) {
        const exists = await prisma_1.prisma.chain.findUnique({
...
router.post('/business', auth_1.requireAuth, async (req, res) => {
    try {
        ...
        if (existingUser) {
            return onboardingErrorResponse(res, 409, new OnboardingError('EMAIL_ALREADY_EXISTS', 'Ein Benutzer mit dieser Admin-E-Mail existiert bereits.'));
        }
        const chainCode = await nextUniqueChainCode(payload.company.name);
        const adminRole = payload.admin.role === 'CHAINADMIN' ? client_1.UserRole.CHAINADMIN : client_1.UserRole.ADMIN;
        const result = await prisma_1.prisma.$transaction(async (tx) => {
            ...
            try {
                ...
                throw new OnboardingError('TENANT_CREATE_FAILED', 'Filiale konnte nicht erstellt werden.', tenantCreateError instanceof Error ? tenantCreateError.message : undefined);
            }
            const adminUser = await tx.user.create({
                data: {
                    name: payload.admin.name.trim(),
                    email: adminEmail,
                    passwordHash: (0, password_1.hashPassword)(payload.admin.password),
                    role: adminRole,
                    isActive: true,
                    chainId: chain.id,
                    tenantId: adminRole === client_1.UserRole.ADMIN ? tenant.id : null,
                },
            });
            let templateImportResult = null;
            if (payload.templateImport?.enabled && payload.templateImport.templateId) {
                try {
                    templateImportResult = await (0, business_template_import_1.importBusinessTemplateToTenant)({
                    ...
                    if (importError instanceof business_template_import_1.TenantImportError && importError.code === 'TENANT_NOT_FOUND') {
                        throw new OnboardingError('TENANT_NOT_FOUND', importError.message, importError.details);
                    }
                    throw new OnboardingError('TEMPLATE_IMPORT_FAILED', 'Vorlage konnte nicht in die Filiale importiert werden.', importError instanceof Error ? importError.message : undefined);
                }
            }
            return {
                chain,
        ...
        if (error instanceof client_1.Prisma.PrismaClientKnownRequestError) {
            if (error.code === 'P2002') {
                return onboardingErrorResponse(res, 409, new OnboardingError('EMAIL_ALREADY_EXISTS', 'E-Mail ist bereits vergeben.', error.message));
            }
            return onboardingErrorResponse(res, 500, new OnboardingError('DATABASE_PROVISIONING_FAILED', 'Datenbankvorgang im Onboarding fehlgeschlagen.', error.message));
        }
        return onboardingErrorResponse(res, 500, new OnboardingError('DATABASE_PROVISIONING_FAILED', 'Onboarding konnte nicht abgeschlossen werden.', error instanceof Error ? error.message : undefined));
    }
});
