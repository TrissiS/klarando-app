...
function parseDays(rawValue: unknown) {
...
router.get('/dashboard', requirePermission(PermissionKey.ORDERS_READ), async (req, res) => {
  try {
    ...
    const chainId = actor.role === UserRole.SUPERADMIN ? requestedChainId : actor.chainId
    if (!chainId) {
    ...
    if (actor.role === UserRole.CHAINADMIN) {
      accessRows = await prisma.chainAdminTenantAccess.findMany({
        where: {
...
router.get('/access-rights', requirePermission(PermissionKey.TENANTS_READ), async (req, res) => {
  try {
    const actor = req.authUser
    if (!actor) {
      return res.status(401).json({ error: 'Nicht eingeloggt' })
    ...
    const requestedUserId = req.query.chainAdminUserId as string | undefined
    ...
    if (actor.role === UserRole.SUPERADMIN) {
      ...
      const chainadminUser = await prisma.user.findUnique({
      ...
      if (!chainadminUser || chainadminUser.role !== UserRole.CHAINADMIN) {
    ...
    } else if (actor.role !== UserRole.CHAINADMIN) {
      return res.status(403).json({ error: 'Nur CHAINADMIN oder SUPERADMIN erlaubt' })
    }
    ...
    const [tenants, rights] = await Promise.all([
      ...
      prisma.chainAdminTenantAccess.findMany({
        where: {
          chainAdminUserId,
          tenant: {
            chainId,
...
router.put('/access-rights', requirePermission(PermissionKey.TENANTS_WRITE), async (req, res) => {
  try {
    const actor = req.authUser
    if (!actor) {
      return res.status(401).json({ error: 'Nicht eingeloggt' })
    ...
    const {
      chainAdminUserId: rawChainAdminUserId,
      tenantId,
      superadminGranted,
      canViewRevenue,
      canManageAppOrders,
    ...
    const existing = await prisma.chainAdminTenantAccess.findUnique({
      where: {
    ...
    if (actor.role === UserRole.CHAINADMIN && (!existing || !existing.superadminGranted)) {
    ...
    const updated = await prisma.chainAdminTenantAccess.upsert({
      where: {
        chainAdminUserId_tenantId: {
          chainAdminUserId,
          tenantId,
      ...
      create: {
        ...
        tenantId,
        superadminGranted:
          actor.role === UserRole.SUPERADMIN ? superadminGranted ?? true : true,
        ...
        canOverrideSettings:
          actor.role === UserRole.SUPERADMIN ? canOverrideSettings ?? false : false,
        notes: normalizeText(notes),
      },
      update: {
        superadminGranted:
          actor.role === UserRole.SUPERADMIN ? superadminGranted : undefined,
        canViewRevenue,
        canManageAppOrders,
        canManageDelivery,
        canManagePickup,
        ...
        canViewStaffPlanning,
        canOverrideSettings:
          actor.role === UserRole.SUPERADMIN ? canOverrideSettings : undefined,
