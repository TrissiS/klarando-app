...
function splitAllergens(value: string | null) {
  if (!value) {
    return []
  }
...
function calculateProduct(product: {
  ...
  const details = product.ingredients.map((item) => {
    ...
    const conversion = convertToPurchaseQuantity({
      ...
      gramsPerPurchaseUnit:
        Number.isFinite(gramsPerPurchaseUnit) && gramsPerPurchaseUnit > 0
          ? gramsPerPurchaseUnit
          : null,
    })
  ...
  const marginPercent =
    safeSellingPriceGross > 0 ? (marginGross / safeSellingPriceGross) * 100 : 0
  const costPercent =
    safeSellingPriceGross > 0 ? (totalCostGross / safeSellingPriceGross) * 100 : 0
...
router.get('/:productId', requirePermission(PermissionKey.PRODUCTS_READ), async (req, res) => {
  try {
