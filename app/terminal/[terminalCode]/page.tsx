...
export default async function LegacyTerminalBlockedPage({ params }: Props) {
    ...
    <main className="flex min-h-screen items-center justify-center bg-slate-950 p-6 text-white">
      <div className="w-full max-w-3xl rounded-2xl border border-red-400/60 bg-red-500/20 p-6">
        ...
        <p className="mt-3 text-sm text-red-100/90">
          Route: <span className="font-mono">/terminal/[terminalCode]</span>
        </p>
        <p className="mt-1 text-sm text-red-100/90">
          terminalCode: <span className="font-mono">{resolved.terminalCode}</span>
        </p>
        <p className="mt-3 text-sm text-red-100/90">
